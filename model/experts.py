...
class Expert(nn.Module):
    def __init__(self, n_embed: int, dropout: float = 0.2):
        ...
        self.expert = nn.Sequential(nn.Linear(n_embed, 2*n_embed),
                                       nn.ReLU(),
...
class SparseMoE(nn.Module):
    def __init__(self, n_experts: int, n_embed: int, top_k: int = 2, dropout: float = 0.2):
        super().__init__()

        self.router = Router(n_embed=n_embed, n_experts=n_experts, top_k=top_k)
        self.experts = [Expert(n_embed=n_embed, dropout=dropout) for _ in range(n_experts)]
    ...
    def __call__(self, x):
        ...
        for i, expert in enumerate(self.experts):
            expert_mask = (indices == i).any(axis=-1) # which batches pass from which experts
            ...
            if flat_mask.any():
                ...
                weighted_output = expert_output * gating_scores.reshape(gating_scores.size, 1)
