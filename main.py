...
model = LanguageModel(vocab_size=vocab_size, n_embed=256, context_len=block_size,
                      n_blocks=6, n_heads=4, n_experts=4, top_k=2, lr=1e-4)
...
for i in bar:
    ...
    if i % save_interval == 0:
        ...
        mx.save_safetensors(f"trained_models/model_{i}_{round(train_loss.item(), 2)}_{round(val_loss.item(), 2)}",
                            dict(flat_params))
